...
_init_c2p_table:
	movem.l %d2-%d7,-(%sp)	|
	lea _c2p_table,%a0	|, ivtmp.56
| c2p.c:10: void init_c2p_table() {
	moveq #0,%d2	| ivtmp.54
| c2p.c:11: 	for (int i=0; i<256; i++) {
	moveq #0,%d0	| i
.L7:
| c2p.c:13: 		if (i & 1) pdata |= 0x01000000;
	move.l %d2,%d1	| ivtmp.54, _167
	and.l #16777216,%d1	|, _167
| c2p.c:14: 		if (i & 2) pdata |= 0x00010000;
	btst #1,%d0	|, i
	jeq .L2		|
| c2p.c:14: 		if (i & 2) pdata |= 0x00010000;
	bset #16,%d1	|, _167
.L2:
| c2p.c:15: 		if (i & 4) pdata |= 0x00000100;
	btst #2,%d0	|, i
	jeq .L3		|
| c2p.c:16: 		if (i & 8) pdata |= 0x00000001;
	btst #3,%d0	|, i
	jne .L4		|
| c2p.c:15: 		if (i & 4) pdata |= 0x00000100;
	or.w #256,%d1	|, _167
.L37:
| c2p.c:17: 		c2p_table[0][i] = pdata;
	move.l %d1,(%a0)+	| _167, MEM[(unsigned int *)_166]
| c2p.c:11: 	for (int i=0; i<256; i++) {
	addq.l #1,%d0	|, i
| c2p.c:11: 	for (int i=0; i<256; i++) {
	add.l #16777216,%d2	|, ivtmp.54
	jra .L7		|
.L3:
| c2p.c:16: 		if (i & 8) pdata |= 0x00000001;
	btst #3,%d0	|, i
...
| c2p.c:16: 		if (i & 8) pdata |= 0x00000001;
	moveq #1,%d3	|,
	or.l %d1,%d3	| _167,
	move.l %d3,(%a0)+	|, MEM[(unsigned int *)_202]
| c2p.c:11: 	for (int i=0; i<256; i++) {
	addq.l #1,%d0	|, i
| c2p.c:11: 	for (int i=0; i<256; i++) {
	add.l #16777216,%d2	|, ivtmp.54
...
| c2p.c:16: 		if (i & 8) pdata |= 0x00000001;
	or.w #257,%d1	|, _167
	move.l %d1,(%a0)+	| _167, MEM[(unsigned int *)_332]
| c2p.c:11: 	for (int i=0; i<256; i++) {
	addq.l #1,%d0	|, i
| c2p.c:11: 	for (int i=0; i<256; i++) {
	add.l #16777216,%d2	|, ivtmp.54
	cmp.l #256,%d0	|, i
	jne .L7		|
	lea _c2p_table+1024,%a0	|, ivtmp.47
| c2p.c:20: 	for (int i=0; i<256; i++) {
	moveq #0,%d2	| i
.L11:
| c2p.c:22: 		unsigned char c = (i & 0xf) + (i >> 4);
	move.l %d2,%d1	| i, _120
	asr.l #4,%d1	|, _120
| c2p.c:22: 		unsigned char c = (i & 0xf) + (i >> 4);
	move.b %d2,%d0	|, _119
	and.b #15,%d0	|, _119
	add.b %d1,%d0	| _120, c
| c2p.c:23: 		if (c & 1) pdata |= 0x01000000;
	move.b %d0,%d1	| c, _123
	and.b #1,%d1	|, _123
	lsl.w #8,%d1	|, pdata
	swap %d1	| pdata
	clr.w %d1	| pdata
| c2p.c:24: 		if (c & 2) pdata |= 0x00010000;
	btst #1,%d0	|, c
	jeq .L8		|
| c2p.c:24: 		if (c & 2) pdata |= 0x00010000;
	bset #16,%d1	|, pdata
.L8:
| c2p.c:25: 		if (c & 4) pdata |= 0x00000100;
	btst #2,%d0	|, c
	jeq .L9		|
| c2p.c:25: 		if (c & 4) pdata |= 0x00000100;
	or.w #256,%d1	|, pdata
.L9:
| c2p.c:26: 		if (c & 8) pdata |= 0x00000001;
	btst #3,%d0	|, c
	jeq .L10		|
| c2p.c:26: 		if (c & 8) pdata |= 0x00000001;
	moveq #1,%d0	|,
	or.l %d0,%d1	|, pdata
.L10:
| c2p.c:27: 		c2p_table[1][i] = pdata;
	move.l %d1,(%a0)+	| pdata, MEM[(unsigned int *)_420]
| c2p.c:20: 	for (int i=0; i<256; i++) {
	addq.l #1,%d2	|, i
| c2p.c:20: 	for (int i=0; i<256; i++) {
	cmp.l #256,%d2	|, i
	jne .L11		|
	lea _c2p_x2_table,%a1	|, ivtmp.38
| c2p.c:30:     for (int i=0; i<256; i++) {
	moveq #0,%d5	| i
.L35:
| c2p.c:31:         unsigned char c1 = i & 0xf;
	move.b %d5,%d3	|, c1
	and.b #15,%d3	|, c1
| c2p.c:32:         unsigned char c2 = (i & 0xf) + (i >> 4);
	move.l %d5,%d2	| i, _11
	asr.l #4,%d2	|, _11
| c2p.c:32:         unsigned char c2 = (i & 0xf) + (i >> 4);
	move.l %a1,%a0	| ivtmp.38, ivtmp.30
	move.b %d3,%d4	| c1, ivtmp.29
| c2p.c:34:         for (unsigned char odd = 0; odd < 2; odd++) {
	clr.b %d1	| odd
.L12:
| c2p.c:41:                     if (c & (1 << (3-bitplane))) {
	move.b %d4,%d0	| ivtmp.29, tmp181
	lsr.b #3,%d0	|, tmp181
	moveq #1,%d6	|,
	and.l %d6,%d0	|, _375
	lsl.l #7,%d0	|, pdata
	btst #2,%d4	|, ivtmp.29
	jeq .L16		|
| c2p.c:42:                         pdata |= (1 << (8*bitplane)) << (7-pixel);
	or.w #32768,%d0	|, pdata
.L16:
| c2p.c:41:                     if (c & (1 << (3-bitplane))) {
	btst #1,%d4	|, ivtmp.29
...
| c2p.c:41:                     if (c & (1 << (3-bitplane))) {
	moveq #1,%d6	|, _195
	and.l %d4,%d6	| ivtmp.29, _195
	neg.l %d6	| tmp188
	and.l #-2147483648,%d6	|, _285
	or.l %d0,%d6	| pdata, pdata
| c2p.c:40:                     unsigned char c = ((odd ^ pixel) & 1) ? c2 : c1;
...
| c2p.c:41:                     if (c & (1 << (3-bitplane))) {
	...
	move.l %d0,(%a0)	| _165, MEM[(unsigned int *)_249]
| c2p.c:40:                     unsigned char c = ((odd ^ pixel) & 1) ? c2 : c1;
...
| c2p.c:41:                     if (c & (1 << (3-bitplane))) {
	...
	or.l %d0,%d6	| _44, _50
	move.l %d6,1024(%a0)	| _50, MEM[(unsigned int *)_249 + 1024B]
| c2p.c:40:                     unsigned char c = ((odd ^ pixel) & 1) ? c2 : c1;
...
| c2p.c:41:                     if (c & (1 << (3-bitplane))) {
	...
	move.l %d6,2048(%a0)	| _382, MEM[(unsigned int *)_249 + 2048B]
| c2p.c:40:                     unsigned char c = ((odd ^ pixel) & 1) ? c2 : c1;
...
| c2p.c:41:                     if (c & (1 << (3-bitplane))) {
	...
	or.l %d0,%d6	| _58, _242
	move.l %d6,3072(%a0)	| _242, MEM[(unsigned int *)_249 + 3072B]
	move.b %d1,%d0	| odd, _56
	addq.b #1,%d0	|, _56
| c2p.c:34:         for (unsigned char odd = 0; odd < 2; odd++) {
	add.b %d2,%d4	| _12, ivtmp.29
	lea (4096,%a0),%a0	|, ivtmp.30
	moveq #1,%d1	|, odd
	cmp.b #2,%d0	|, _56
	jne .L12		|
| c2p.c:30:     for (int i=0; i<256; i++) {
...
.L114:
| c2p.c:58: 				pdata |= c2p_table[odd&1][*in++];
	move.l %d1,%d0	|, _163
	lsl.l #8,%d0	|, _163
| c2p.c:58: 				pdata |= c2p_table[odd&1][*in++];
	moveq #0,%d3	| _166
...
| c2p.c:69:     unsigned int (*table)[4][256] = c2p_x2_table + (odd & 1);
...
| c2p.c:75:                 unsigned char color = *in++;
	addq.l #8,%a0	|, in
| c2p.c:76: 				pdata |= (*table)[i][color];
	moveq #0,%d0	| _108
...
| c2p.c:87: 	const unsigned int *table = c2p_table[odd & 1];
	moveq #1,%d2	|, _21
	and.l %d0,%d2	| odd, _21
	moveq #10,%d3	|,
	lsl.l %d3,%d2	|, _14
...
| c2p.c:88: 	const unsigned int *other = c2p_table[!odd & 1];
	tst.b %d0	| odd
	seq %d0		| tmp198
	ext.w %d0	| tmp199
	ext.l %d0	| tmp197
...
| c2p.c:89:     unsigned short groups = pixels >> 4;
	move.l 32(%sp),%d0	| pixels, pixels
	lsr.w #4,%d0	|, groups
| c2p.c:90: 	while (groups-- > 0) {
