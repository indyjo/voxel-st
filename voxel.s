...
| voxel.c:506: 	unsigned int movep_data = pdata_table[color][0][0];
	moveq #0,%d0	| color
...
| voxel.c:680: 	printf("\n");
	pea 10.w		|
	jsr (%a3)		| tmp110
| voxel.c:681: 	free_image(&height);
	pea 62(%sp)		|
	jsr (%a5)		| tmp104
| voxel.c:683: 	fclose(file1);
	move.l %d3,-(%sp)	| file1,
	lea _fclose,%a2	|, tmp99
	jsr (%a2)		| tmp99
| voxel.c:684: 	fclose(file2);
	move.l %d6,-(%sp)	| file2,
	jsr (%a2)		| tmp99
| voxel.c:685: 	return 1; // success
	lea (16,%sp),%sp	|,
	moveq #1,%d0	|, <retval>
| voxel.c:693: }
	movem.l (%sp)+,#31996	|,
...
_get_key:
	movem.l #12320,-(%sp)	|,
| voxel.c:696: 	return Bconstat(_CON) ? (Bconin(_CON) & 0xff0000) >> 16 : 0;
	moveq #2,%d3	|, tmp36
#APP
| 696 "voxel.c" 1
	movw	%d3,%sp@-	| tmp36
...
_main:
	lea (-120,%sp),%sp	|,
	movem.l #16190,-(%sp)	|,
| voxel.c:709: int main(int argc, char **argv) {
	jsr ___main		|
| voxel.c:711: 	printf("\33H\33f\n\n");
	pea .LC6		|
	jsr _puts		|
| voxel.c:714: 	Super(0L);
	moveq #0,%d0	| tmp583
#APP
| 714 "voxel.c" 1
	movl	%d0,%sp@-	| tmp583
...
| voxel.c:718: 	lineaa();
	jsr _lineaa		|
| voxel.c:721: 	save_palette(saved_palette);
	pea 136(%sp)		|
	jsr _save_palette		|
| voxel.c:723: 	unsigned short *screen = Physbase();
#APP
| 723 "voxel.c" 1
	movw	#2,%sp@-	|
	trap	#14
...
| voxel.c:725: 	*conterm &= ~1;
	and.b #-2,1156.w	|, MEM[(char *)1156B]
| voxel.c:727: 	if (!load_voxel_data()) {
	jsr _load_voxel_data		|
| voxel.c:727: 	if (!load_voxel_data()) {
	addq.l #8,%sp	|,
	tst.l %d0	| tmp996
	jeq .L348		|
| voxel.c:732: 	printf("Computing tables\n");
	pea .LC8		|
	jsr _puts		|
| voxel.c:733: 	build_tables();
	jsr _build_tables		|
| voxel.c:734: 	printf("Loading cockpit.tga\n");
	pea .LC9		|
	jsr _puts		|
| voxel.c:735: 	image_t cockpit = read_tga("cockpit.tga");
	pea .LC10		|
	lea (130,%sp),%a1	|,,
	jsr _read_tga		|
	move.l 134(%sp),%d6	| cockpit.pixels, cockpit$pixels
| voxel.c:736: 	if (!cockpit.pixels) goto error;
	lea (12,%sp),%sp	|,
	jeq .L208		|
| voxel.c:737: 	read_palette_vectors(cockpit.colors);
	move.l 126(%sp),-(%sp)	| cockpit.colors,
	jsr _read_palette_vectors		|
	move.l 60(%sp),%a0	| %sfp, ivtmp.740
	move.l %a0,%d0	| ivtmp.740, _1534
	add.l #32000,%d0	|, _1534
	addq.l #4,%sp	|,
.L209:
| voxel.c:147: 	for (int i=0; i<16000; i++) *out++ = 0;
	clr.l (%a0)+	| MEM <vector(2) short unsigned int> [(short unsigned int *)vectp.521_1670]
	cmp.l %d0,%a0	| _1534, ivtmp.740
	jne .L209		|
| voxel.c:506: 	unsigned int movep_data = pdata_table[color][0][0];
	...
	move.l #_view_min+80,%d4	|, _1540
	clr.w %d3	| ivtmp.732
	move.l 56(%sp),%a1	| %sfp, _201
.L214:
| voxel.c:741: 		fill_column(screen, i*8, 0, view_min[i], 0);
	move.w (%a2)+,%d1	| MEM[(short int *)_1542], _6
| voxel.c:338: 	return ((unsigned char *)&out[y*80 + ((x>>4)<<2)]) + ((x >> 3) & 1);
	move.w %d3,%d5	| ivtmp.732, tmp599
	asr.w #4,%d5	|, tmp599
	ext.l %d5	| tmp600
| voxel.c:338: 	return ((unsigned char *)&out[y*80 + ((x>>4)<<2)]) + ((x >> 3) & 1);
	add.l %d5,%d5	| tmp600, tmp601
	add.l %d5,%d5	| tmp601, _297
| voxel.c:338: 	return ((unsigned char *)&out[y*80 + ((x>>4)<<2)]) + ((x >> 3) & 1);
	move.l %d3,%d2	|, tmp604
	lsr.l #3,%d2	|, tmp604
	moveq #1,%d7	|,
	and.l %d7,%d2	|, _302
| voxel.c:338: 	return ((unsigned char *)&out[y*80 + ((x>>4)<<2)]) + ((x >> 3) & 1);
	move.l %d5,%a0	| _297, tmp605
	add.l %d5,%a0	| _297, tmp605
| voxel.c:338: 	return ((unsigned char *)&out[y*80 + ((x>>4)<<2)]) + ((x >> 3) & 1);
	add.l %d2,%a0	| _302, tmp606
	lea (%a1,%a0.l),%a0	| _201, tmp606, pBlock
| voxel.c:507: 	for (short remaining = height; remaining > 0; remaining -= LINES_SKIP) {
	tst.w %d1	| _6
	jle .L210		|
	mulu.w #160,%d1	|, tmp607
	add.l %a0,%d1	| pBlock, _1570
.L211:
| voxel.c:343: 	asm ("movep.l %0, 0(%1)" : : "d" (data), "a" (p));
#APP
| 343 "voxel.c" 1
...
#NO_APP
	lea (160,%a0),%a0	|, pBlock
| voxel.c:507: 	for (short remaining = height; remaining > 0; remaining -= LINES_SKIP) {
	cmp.l %a0,%d1	| pBlock, _1570
	jne .L211		|
.L210:
| voxel.c:742: 		fill_column(screen, i*8, view_max[i]+1, 199 - view_max[i], 0);
	move.w (%a3)+,%a0	| MEM[(short int *)_1539], _10
| voxel.c:742: 		fill_column(screen, i*8, view_max[i]+1, 199 - view_max[i], 0);
	move.w #199,%d1	|, _12
	sub.w %a0,%d1	| _10, _12
| voxel.c:742: 		fill_column(screen, i*8, view_max[i]+1, 199 - view_max[i], 0);
...
| voxel.c:338: 	return ((unsigned char *)&out[y*80 + ((x>>4)<<2)]) + ((x >> 3) & 1);
	...
	move.l %d7,%a0	|, tmp610
| voxel.c:338: 	return ((unsigned char *)&out[y*80 + ((x>>4)<<2)]) + ((x >> 3) & 1);
	add.l %d5,%a0	| _297, tmp612
| voxel.c:338: 	return ((unsigned char *)&out[y*80 + ((x>>4)<<2)]) + ((x >> 3) & 1);
	add.l %a0,%a0	| tmp612, tmp613
| voxel.c:338: 	return ((unsigned char *)&out[y*80 + ((x>>4)<<2)]) + ((x >> 3) & 1);
	add.l %a0,%d2	| tmp613, tmp614
	lea (%a1,%d2.l),%a0	| _201, tmp614, pBlock
| voxel.c:507: 	for (short remaining = height; remaining > 0; remaining -= LINES_SKIP) {
	tst.w %d1	| _12
	jle .L212		|
	mulu.w #160,%d1	|, tmp615
...
| voxel.c:507: 	for (short remaining = height; remaining > 0; remaining -= LINES_SKIP) {
	...
	jne .L213		|
.L212:
| voxel.c:740: 	for (int i=0; i<40; i++) {
	addq.w #8,%d3	|, ivtmp.732
	cmp.l %d4,%a2	| _1540, ivtmp.731
	jne .L214		|
| voxel.c:745: 	compute_and_set_bottom_palette(0, 0);
	clr.l -(%sp)	|
	clr.l -(%sp)	|
	jsr _compute_and_set_bottom_palette		|
| voxel.c:746: 	install_joystick_handler();
	jsr _install_joystick_handler		|
| voxel.c:747: 	install_interrupts();
	jsr _install_interrupts		|
| voxel.c:750: 	draw_image2(screen + cockpit_y*80, cockpit.pixels, cockpit.width, 200 - cockpit_y, 0);
	clr.l -(%sp)	|
	pea 80.w		|
	...
	pea 19200(%a1)		|
	jsr _draw_image2		|
| voxel.c:754: 	unsigned long t0 = *_hz_200;
	move.l 1210.w,142(%sp)	| MEM[(volatile long unsigned int *)1210B], %sfp
	lea (28,%sp),%sp	|,
	clr.w 94(%sp)	| %sfp
| voxel.c:759: 	int frames = 0;
	clr.l 86(%sp)	| %sfp
| voxel.c:757: 	fixp desired_height = FIXP(20, 0);
	move.w #2560,84(%sp)	|, %sfp
| voxel.c:755: 	unsigned long t_render = 0;
	clr.l 90(%sp)	| %sfp
| voxel.c:752: 	char fog_enabled = FOG_ENABLED_INITIALLY;
	moveq #1,%d7	|, fog_enabled
| voxel.c:772: 		fixp terrain_height = FIXP(combined[fixp_int(pos.y)][fixp_int(pos.x)].height >> 1, 0);
...
#NO_APP
	...
	ext.l %d0	| tmp627
| voxel.c:696: 	return Bconstat(_CON) ? (Bconin(_CON) & 0xff0000) >> 16 : 0;
	move.b %d0,105(%sp)	| tmp627, %sfp
| voxel.c:763: 		if (key == 1)
	cmp.b #1,%d0	|,
	jeq .L349		|
.L215:
| voxel.c:768: 		frames++;
...
| voxel.c:773: 		fixp player_height = pos.z - terrain_height;
	move.w _pos+4,%d2	| pos.z, _28
	move.w %d2,%d5	| _28, pretmp_1566
| voxel.c:155: 	return ((unsigned short)val) >> FIXP_PRECISION;
	move.w _pos+2,%d0	| pos.y, pos.y
	lsr.w #7,%d0	|, tmp632
| voxel.c:772: 		fixp terrain_height = FIXP(combined[fixp_int(pos.y)][fixp_int(pos.x)].height >> 1, 0);
	and.l #65535,%d0	|, tmp634
| voxel.c:155: 	return ((unsigned short)val) >> FIXP_PRECISION;
	move.w (%a0),%d1	| pos.x, pos.x
	lsr.w #7,%d1	|, tmp636
| voxel.c:772: 		fixp terrain_height = FIXP(combined[fixp_int(pos.y)][fixp_int(pos.x)].height >> 1, 0);
	and.l #65535,%d1	|, tmp638
	moveq #9,%d3	|,
	lsl.l %d3,%d0	|, tmp639
	add.l %d1,%d0	| tmp638, tmp640
...
| voxel.c:774: 		if (desired_height >= 0) {
	tst.w 84(%sp)	| %sfp
	jlt .L217		|
| voxel.c:775: 			fixp altitude_delta = (desired_height - player_height) / (desired_height >> 9);
	move.w 84(%sp),%a0	| %sfp, desired_height
| voxel.c:775: 			fixp altitude_delta = (desired_height - player_height) / (desired_height >> 9);
	move.w 84(%sp),%d0	| %sfp, tmp652
	moveq #9,%d5	|,
	asr.w %d5,%d0	|, tmp652
| voxel.c:775: 			fixp altitude_delta = (desired_height - player_height) / (desired_height >> 9);
	move.w %d0,%a1	| tmp652,
	...
	jsr ___divsi3		|
	addq.l #8,%sp	|,
| voxel.c:777: 			if (altitude_delta < -FIXP(2, 0)) altitude_delta = -FIXP(2, 0);
	cmp.w #-256,%d0	|, altitude_delta
	jge .L218		|
	move.w #-256,%d0	|, altitude_delta
.L219:
| voxel.c:778: 			pos.z += altitude_delta;
	move.w %d0,%d5	| altitude_delta, pretmp_1566
	add.w %d2,%d5	| _28, pretmp_1566
| voxel.c:779: 			if (pos.z > FIXP(255, 0)) pos.z = FIXP(255, 0);
	cmp.w #32640,%d5	|, pretmp_1566
	jgt .L220		|
.L354:
| voxel.c:778: 			pos.z += altitude_delta;
	move.w %d5,_pos+4	| pretmp_1566, pos.z
.L217:
| voxel.c:782: 		short mouse_x = GCURX, mouse_y = GCURY;
...
| voxel.c:794: 			int y_offset = ((mouse_y - 100) >> 2) - ((mouse_x - 160) >> 2) * (x-160) / 160;
	asr.l #2,%d0	|, tmp662
	move.l %d0,76(%sp)	| tmp662, %sfp
| voxel.c:794: 			int y_offset = ((mouse_y - 100) >> 2) - ((mouse_x - 160) >> 2) * (x-160) / 160;
	move.w 100(%sp),%a0	| %sfp, mouse_x
	move.l %a0,%d0	| mouse_x, tmp664
	add.l #-160,%d0	|, tmp664
| voxel.c:794: 			int y_offset = ((mouse_y - 100) >> 2) - ((mouse_x - 160) >> 2) * (x-160) / 160;
	asr.l #2,%d0	|, _49
	move.l %d0,%d1	| _49,
	...
	add.l %d2,%d2	| x, tmp671
	add.l #_horizon,%d2	|, tmp671
	move.l %d2,48(%sp)	| tmp671, %sfp
...
| voxel.c:800: 				pos.dirx - ((short)(x - 160) * pos.diry >> 8),
	...
	add.w #-160,%d0	|, _57
	move.w %d0,%d2	| _57,
	muls.w %d1,%d2	| _54, tmp674
| voxel.c:800: 				pos.dirx - ((short)(x - 160) * pos.diry >> 8),
	asr.l #8,%d2	|, tmp675
| voxel.c:800: 				pos.dirx - ((short)(x - 160) * pos.diry >> 8),
	move.w %d4,%d3	| _59, tmp676
	sub.w %d2,%d3	| tmp675, tmp676
| voxel.c:175: 	return (((unsigned int)(unsigned short)a << 16) | (unsigned short)b) & 0xfffefffe;
	swap %d3	| tmp678
	clr.w %d3	| tmp678
| voxel.c:801: 				pos.diry + ((short)(x - 160) * pos.dirx >> 8));
	muls.w %d4,%d0	| _59, tmp679
| voxel.c:801: 				pos.diry + ((short)(x - 160) * pos.dirx >> 8));
	asr.l #8,%d0	|, tmp680
| voxel.c:801: 				pos.diry + ((short)(x - 160) * pos.dirx >> 8));
	add.w %d0,%d1	| tmp680, tmp681
| voxel.c:175: 	return (((unsigned int)(unsigned short)a << 16) | (unsigned short)b) & 0xfffefffe;
	or.w %d1,%d3	| tmp681, tmp683
| voxel.c:175: 	return (((unsigned int)(unsigned short)a << 16) | (unsigned short)b) & 0xfffefffe;
	and.l #-65538,%d3	|, _330
| voxel.c:179: 	return (a + b) & 0xfffefffe;
	move.l 72(%sp),%d2	| %sfp, tmp684
	add.l %d3,%d2	| _330, tmp684
| voxel.c:179: 	return (a + b) & 0xfffefffe;
	and.l #-65538,%d2	|, tmp685
| voxel.c:179: 	return (a + b) & 0xfffefffe;
	add.l %d3,%d2	| _330, tmp686
| voxel.c:179: 	return (a + b) & 0xfffefffe;
	and.l #-65538,%d2	|, tmp687
| voxel.c:179: 	return (a + b) & 0xfffefffe;
	add.l %d3,%d2	| _330, tmp688
| voxel.c:179: 	return (a + b) & 0xfffefffe;
	and.l #-65538,%d2	|, tmp689
| voxel.c:179: 	return (a + b) & 0xfffefffe;
	add.l %d3,%d2	| _330, tmp690
| voxel.c:179: 	return (a + b) & 0xfffefffe;
	and.l #-65538,%d2	|, sample_uv
| voxel.c:810: 				.pixel = pixel_block_address(screen, x, view_max[x >> 3]),
	move.w %d6,%d1	| x, tmp691
	lsr.w #3,%d1	|, tmp691
	and.l #65535,%d1	|,
	move.l %d1,%a4	|, _72
| voxel.c:810: 				.pixel = pixel_block_address(screen, x, view_max[x >> 3]),
	add.l %d1,%a4	| _72, tmp693
	lea _view_max,%a1	|,
	move.w (%a1,%a4.l),%a3	| view_max[_72], _1690
| voxel.c:338: 	return ((unsigned char *)&out[y*80 + ((x>>4)<<2)]) + ((x >> 3) & 1);
	move.w %d6,%d0	| x, tmp694
	asr.w #4,%d0	|, tmp694
	ext.l %d0	| tmp695
| voxel.c:338: 	return ((unsigned char *)&out[y*80 + ((x>>4)<<2)]) + ((x >> 3) & 1);
	add.l %d0,%d0	| tmp695, tmp696
	add.l %d0,%d0	| tmp696, tmp696
	move.l %d0,64(%sp)	| tmp696, %sfp
| voxel.c:338: 	return ((unsigned char *)&out[y*80 + ((x>>4)<<2)]) + ((x >> 3) & 1);
	move.l %d6,%d0	|, tmp699
	lsr.l #3,%d0	|, tmp699
	moveq #1,%d1	|,
...
| voxel.c:338: 	return ((unsigned char *)&out[y*80 + ((x>>4)<<2)]) + ((x >> 3) & 1);
	...
	muls.w #80,%d0	|, tmp700
| voxel.c:338: 	return ((unsigned char *)&out[y*80 + ((x>>4)<<2)]) + ((x >> 3) & 1);
	add.l 64(%sp),%d0	| %sfp, tmp702
| voxel.c:338: 	return ((unsigned char *)&out[y*80 + ((x>>4)<<2)]) + ((x >> 3) & 1);
	add.l %d0,%d0	| tmp702, tmp703
| voxel.c:338: 	return ((unsigned char *)&out[y*80 + ((x>>4)<<2)]) + ((x >> 3) & 1);
	add.l %d1,%d0	|, tmp704
	move.l 56(%sp),%a2	| %sfp, pBlock
	add.l %d0,%a2	| tmp704, pBlock
| voxel.c:809: 				.y = view_max[x >> 3] - y_offset,
	move.w %a3,%d4	| _1690, _1686
| voxel.c:794: 			int y_offset = ((mouse_y - 100) >> 2) - ((mouse_x - 160) >> 2) * (x-160) / 160;
	pea -160.w		|
	move.l 64(%sp),-(%sp)	| %sfp,
	jsr ___divsi3		|
...
| voxel.c:816: 			asm (".globl  _begin_render_column");
#APP
| 816 "voxel.c" 1
	.globl  _begin_render_column
| 0 "" 2
| voxel.c:817: 			asm ("_begin_render_column:");
| 817 "voxel.c" 1
...
| voxel.c:411: 	short ytable_offset = 256 - player_height;
	...
	sub.w %d5,%d1	| tmp713, ytable_offset
...
| voxel.c:831: 			state.y += y_offset;
	move.w %d0,%d4	| y, _1686
	add.w 54(%sp),%d4	| %sfp, _1686
	move.w %d4,%a3	| _1686, _1690
.L276:
| voxel.c:515: 	fill_column(out, x, horizon[x] + LINES_SKIP, y - horizon[x], 15);
	move.l 48(%sp),%a0	| %sfp,
	move.w (%a0),%d0	| MEM[(short int *)_1308], _403
| voxel.c:515: 	fill_column(out, x, horizon[x] + LINES_SKIP, y - horizon[x], 15);
...
| voxel.c:338: 	return ((unsigned char *)&out[y*80 + ((x>>4)<<2)]) + ((x >> 3) & 1);
	muls.w #80,%d0	|, tmp804
| voxel.c:338: 	return ((unsigned char *)&out[y*80 + ((x>>4)<<2)]) + ((x >> 3) & 1);
	add.l 64(%sp),%d0	| %sfp, tmp806
| voxel.c:338: 	return ((unsigned char *)&out[y*80 + ((x>>4)<<2)]) + ((x >> 3) & 1);
	add.l %d0,%d0	| tmp806, tmp807
| voxel.c:338: 	return ((unsigned char *)&out[y*80 + ((x>>4)<<2)]) + ((x >> 3) & 1);
	add.l 68(%sp),%d0	| %sfp, tmp808
	move.l 56(%sp),%a0	| %sfp, pBlock
	add.l %d0,%a0	| tmp808, pBlock
| voxel.c:506: 	unsigned int movep_data = pdata_table[color][0][0];
	move.l _pdata_table+3840,%d0	| pdata_table[15][0][0], movep_data
| voxel.c:507: 	for (short remaining = height; remaining > 0; remaining -= LINES_SKIP) {
	tst.w %d4	| _404
	jle .L278		|
	mulu.w #160,%d4	|, tmp810
	add.l %a0,%d4	| pBlock, _1639
.L279:
| voxel.c:343: 	asm ("movep.l %0, 0(%1)" : : "d" (data), "a" (p));
#APP
| 343 "voxel.c" 1
...
#NO_APP
	lea (160,%a0),%a0	|, pBlock
| voxel.c:507: 	for (short remaining = height; remaining > 0; remaining -= LINES_SKIP) {
	cmp.l %a0,%d4	| pBlock, _1639
	jne .L279		|
.L278:
| voxel.c:516: 	horizon[x] = y;
	move.l 48(%sp),%a1	| %sfp,
	move.w %a3,(%a1)	| _1690, MEM[(short int *)_1308]
| voxel.c:833: 			asm (".globl  _end_render_column");
#APP
| 833 "voxel.c" 1
	.globl  _end_render_column
| 0 "" 2
| voxel.c:834: 			asm ("_end_render_column:");
| 834 "voxel.c" 1
	_end_render_column:
| 0 "" 2
| voxel.c:788: 		for (unsigned short x = VIEWPORT_MIN + 3 + ((i&1)<<3); x < VIEWPORT_MAX; x += 16) {
...
| voxel.c:788: 		for (unsigned short x = VIEWPORT_MIN + 3 + ((i&1)<<3); x < VIEWPORT_MAX; x += 16) {
	...
	cmp.w #319,%d6	|, x
	jhi .L280		|
| voxel.c:842: 		short elev_to_sun = ray_elevation(player_uv, make_2in1(0, -FIXP(1, 0)), fixp_int(pos.z));
	move.w _pos+4,%d5	| pos.z, pretmp_1566
	jra .L281		|
.L349:
...
| voxel.c:441: 					unsigned int movep_data = pdata_entry[y&7];
	moveq #7,%d1	|, tmp760
	and.l %d0,%d1	| y, tmp760
	add.l %d1,%d1	| tmp760, tmp761
...
| voxel.c:382: 	unsigned short offset = (sample.both & 0xff00) + opacity_preshifted;
	clr.w %d5	| opacity_preshifted
	move.b (%a1),%d5	| MEM[(unsigned char *)fog_table_shifted_1296], opacity_preshifted
	move.w %d5,%a3	| opacity_preshifted, offset
	add.w %d1,%a3	| _1795, offset
| voxel.c:383: 	return (unsigned int *)((char*)pdata_table + offset);
	moveq #0,%d1	| _476
...
| voxel.c:441: 					unsigned int movep_data = pdata_entry[y&7];
	moveq #7,%d1	|, tmp795
	and.l %d0,%d1	| y, tmp795
	add.l %d1,%d1	| tmp795, tmp796
...
| voxel.c:382: 	unsigned short offset = (sample.both & 0xff00) + opacity_preshifted;
	clr.w %d5	| opacity_preshifted
	move.b (%a1),%d5	| MEM[(unsigned char *)fog_table_shifted_1053], opacity_preshifted
	move.w %d5,%a3	| opacity_preshifted, offset
	add.w %d1,%a3	| _1797, offset
| voxel.c:383: 	return (unsigned int *)((char*)pdata_table + offset);
	moveq #0,%d1	| _551
...
| voxel.c:441: 					unsigned int movep_data = pdata_entry[y&7];
	moveq #7,%d1	|, tmp778
	and.l %d0,%d1	| y, tmp778
	add.l %d1,%d1	| tmp778, tmp779
...
| voxel.c:838: 		t_render += t_render_1 - t_render_0;
	sub.l 110(%sp),%d0	| %sfp, tmp814
| voxel.c:838: 		t_render += t_render_1 - t_render_0;
	add.l %d0,90(%sp)	| tmp814, %sfp
| voxel.c:155: 	return ((unsigned short)val) >> FIXP_PRECISION;
	move.w _pos+4,%d0	| pos.z, pos.z
	lsr.w #7,%d0	|, tmp816
| voxel.c:470: 	short ytable_offset = 256 - start_height;
	move.w #256,%a0	|, ytable_offset
	sub.w %d0,%a0	| tmp816, ytable_offset
| voxel.c:472: 	short (*y_table_shifted)[HEIGHT_VALUES] = (short (*)[HEIGHT_VALUES])(y_table[0] + ytable_offset);
	move.w %a0,%a0	| ytable_offset, ytable_offset
	add.l %a0,%a0	| ytable_offset, tmp820
| voxel.c:472: 	short (*y_table_shifted)[HEIGHT_VALUES] = (short (*)[HEIGHT_VALUES])(y_table[0] + ytable_offset);
...
| voxel.c:179: 	return (a + b) & 0xfffefffe;
	move.l %d2,%d6	| delta_uv, tmp821
	add.l %d0,%d6	| uv, tmp821
| voxel.c:179: 	return (a + b) & 0xfffefffe;
	and.l #-65538,%d6	|, sample_uv
...
| voxel.c:483: 			short sample_y = y_table_shifted[0][sample.height];
	and.l #255,%d0	|, SR.519
| voxel.c:483: 			short sample_y = y_table_shifted[0][sample.height];
	add.l %d0,%d0	| SR.519, tmp826
| voxel.c:484: 			if (sample_y < min_y) {
	move.w (%a1,%d0.l),%d0	| (*y_table_shifted_1219)[_963], (*y_table_shifted_1219)[_963]
	cmp.w %d5,%d0	| min_y, (*y_table_shifted_1219)[_963]
	jge .L283		|
	move.w %d0,%d5	| (*y_table_shifted_1219)[_963], min_y
.L283:
| voxel.c:492: 		if (TRIGGERS_PROGRESSION(z)) {
...
| voxel.c:874: 			desired_height += FIXP(1, 0);
	move.w 84(%sp),%d0	| %sfp, _1714
.L289:
	add.w #128,%d0	|, _1714
	move.w %d0,84(%sp)	| _1714, %sfp
...
| voxel.c:201: 	data |= (color & 1) << (15-x);
	move.l #536870912,%d1	|, _1634
	move.w %d0,%d1	| _1634, _1634
| voxel.c:200: 	data &= mask;
...
| voxel.c:201: 	data |= (color & 1) << (15-x);
	move.l #536870912,%d1	|, _1762
	move.w %d0,%d1	| _1762, _1762
| voxel.c:200: 	data &= mask;
...
| voxel.c:768: 		frames++;
	...
	jra .L353		|
.L306:
	move.w 102(%sp),%d0	| %sfp, _1714
	jra .L289		|
.L348:
| voxel.c:728: 		printf("Failed to load voxel data.\n");
	pea .LC7		|
	jsr _puts		|
| voxel.c:729: 		goto error;
	addq.l #4,%sp	|,
| voxel.c:921: 	printf("Press any key to exit to TOS.\n");
	pea .LC13		|
	jsr _puts		|
